...
append_server_override_if_set() {
  local name="$1"
  if [ "${!name+x}" = "x" ]; then
    server_env_base+=("${name}=${!name}")
  fi
}

# Inherit the env-file for settled DSpark and kernel defaults. These optional
# overrides are only passed through when the caller explicitly sets them for a
# specific scaling run.
for name in \
  ...
  VLLM_DSPARK_CONFIDENCE_DIAGNOSTICS_LOG_EVERY \
  ...
  VLLM_DSPARK_DRAFT_STREAM; do
  append_server_override_if_set "${name}"
done

...
server_env_base=(
  ...
  "VLLM_LOG_STATS_INTERVAL=${VLLM_LOG_STATS_INTERVAL:-10.0}"
  ...
  "VLLM_DSPARK_STS_CALIBRATION_DIAGNOSTICS=${VLLM_DSPARK_STS_CALIBRATION_DIAGNOSTICS:-0}"
  ...
  "VLLM_DSPARK_GPU_REJECTED_CONTEXT_MASK=${VLLM_DSPARK_GPU_REJECTED_CONTEXT_MASK:-1}"
...
docker inspect vllm-dspark-head --format '{{range .Config.Env}}{{println .}}{{end}}' \
  | sort \
  | grep -E 'MAX_NUM_SEQS|MAX_MODEL_LEN|MAX_NUM_BATCHED|DSPARK|B12X|ALLREDUCE|FLASHINFER|SERVED_MODEL_NAME' \
